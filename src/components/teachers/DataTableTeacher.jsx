...
import { getAllTeachers, searchTeacher } from '../../features/teacherSlice';
...
export default function DataTableTeacher({
  data,
  columns,
  isLoading,
  onRefresh,
  ...
  useEffect(() => {
    if (search && search.length > 0) {
      dispatch(searchTeacher(search)).unwrap();
    }
    else {
      dispatch(getAllTeachers()).unwrap();
    }
  }, [search, dispatch]);
        ...
        >
          <Stack py={2} direction={["column", "row", "row"]} gap={2} alignItems="center" justifyContent={'space-between'} display={'flex'} width={'100%'}>
            <TextField
              ...
              size="small"
              variant="outlined"
              label="BUSCAR DOCENTE POR NOMBRE, DNI O CORREO"
              value={search}
                  ...
                  {headerGroup.headers.map((header) => (
                    <TableCell
                      ...
                      sx={{
                        ...
                        // Aplicar estilos sticky si corresponde
